...
权限设计：用户，角色，权限，三者之间的关系，用户和角色是多对多，角色和权限也是多对多的设计。

权限框架：shiro，springsecurity。

前后端分离是否任何知道了你的url接口地址的都可以请求？，我做的是判断用户是否具有权限

2.分布式事务的解决方案？最终一致性，消息最终可靠，最大努力通知
...
（1）分库分表，你怎么确定我现在分出来的表数据我要去查询时怎么确定这个数据在哪张表里面？

分表策略： 如按照id范围，id1-1万我放到第一张表，1万到2万方第二张表；按时间来放，今年的或者这个月的放到一张表；hash算法，通过一个标识唯一的能算出一个表名，把他存进去，我查询的时候也是通过同样的标识算法，找到该表。

（2）分完表后现在我查询某一个 范围跨表了，怎么解决？

通过标识的范围计算出他到底在哪几张表，然后分别查询出来，在sql中有个语句叫union组合起来；还有别的吗？
（3）union和unionall有什么区别？

在组合时，unionall是可以哟重复的，union是没有重复的
...
1.2 List	常用命令：lpush,rpush,lpop,rpop,lrange

1.3 hash	常用命令：hget,hset,hgetall 等。

1.4 set    常用命令：sadd,spop,smembers,sunion 等。

1.5sortSet	常用命令：zadd,zrange,zrem,zcard等

2.zookeeper分布式锁解决redis缓存击穿问题？

2.1 缓存穿透：redis没有命中热点数据。

数据库遭到攻击的场景：01：频繁访问一个不存在的值。  解决方案：将第一次查询出来的null值缓存导redis中

02：如果每次访问的id不同还是采用将查询出的第一个值缓存到redis中会适得其反，因为由于redis的内存淘汰策略会导致这些攻击id在内存中占据的热点数据越来越多，导致真正有用的数据被淘汰掉。

针对第二种攻击，可以采用过滤器，如果数据量很小，普通过滤器就可以解决，但是如果数据量很大，使用普通过滤器会导致内存紧张，这是就可以采用布隆过滤器来解决。

布隆算法：通过一定的错误率。这个错误率可能是有hash碰撞导致的，如果经过hash运算后标记值存在那么真实的情况是有可能不存在，如果他告诉你数据不存在，那么就一定不存在。

怎么降低错误率：1.增加bit数组长度。2.增加hash函数的个数（要有度）。

2.2缓存雪崩：redis中缓存大量热点数据，在某一时刻突然集中失效。

造成雪崩的主要原因：1.redis中的数据，有效期都是一样的。解决：给每条数据设置一个随机有效期，给点缓冲时间。2.Redis突然挂掉了。解决：做高可用分布式缓存

2.3缓存击穿：redis只存储一条数据，某一刻数据失效，实质是缓存穿透，中小型项目不需要解决。只有大型项目才解决。解决方案使用分布式锁。

**102.hashMap相关？**

1.hsahMap在jdk1.7时使用的是数组+链表的结构，加上链表是为了减少hash碰撞，但是一旦链表长度过大会导致查询效率很低，因此在jdk1.8后数据结构改成了数组+链表+红黑树的结构用来增加查询效率。

2.hashMap的默认的初始容量是16，即使你设置的初始容量是11它在底层也会给你强转成比11大的最近的2的n次方的值如16。

3.为什么初始容量要是2的指数次幂呢？为了增大hash值的散列程度，他在底层使用的是二进制的移位和异或运算要经历四次移位和四次异或。这样能极大的增加hash值得散列程度，减少hash碰撞。

4.Java1.7中扩容时会产生死锁（使用的是头插法），Java1.8得到了改善。

**103.线程池的受用场景？**

**104.MQ消息中间件？**

1.rocketMQ参照kafka性能可达50w/tps,通过配置优化可以保证消息的0丢失，消息重复消费。

2.RabbitMq，性能相对差点<10w/tps,社区活跃，会有消息丢失。

3.Kafa-性能最好100w-日志收集。
